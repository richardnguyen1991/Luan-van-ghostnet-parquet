...
The CLI default is 100 epochs. Step 8 uses short-lived, object-scoped presigned
PUT/GET URLs generated by GitHub Actions. Long-lived AWS credentials remain in
GitHub Secrets and are never copied to Kaggle Secrets or notebook source.
...
The bundled private Kaggle notebook uses the dataset attached under
`/kaggle/input`. It never requests `KAGGLE_API_TOKEN` or a long-lived AWS key.
If the input is absent, the setup cell stops with an explicit **Add Input**
instruction.
...
The GitHub workflow `.github/workflows/run-kaggle.yml` is the Step 8 watchdog.
It runs every 30 minutes (and manually), checks Kaggle plus S3 state, and starts
only one CPU session when the previous session is no longer active and the run
is incomplete. Progress guards prevent duplicate and repeatedly stagnant
launches.
...
GitHub uses the AWS secrets only to inspect the run state and generate 24-hour
presigned URLs. Kaggle writes directly to
`s3://<S3_BUCKET>/<S3_PREFIX>/gc-lstm-ghostnet/<run_id>/` and updates
`<S3_PREFIX>/active_run.json`; therefore a safe stop can resume in the next
Kaggle session without uploading a checkpoint dataset manually.
...
The automated notebook always uses `--resume auto`: it downloads
`checkpoints/last_checkpoint.pt` through a presigned GET URL when S3 indicates
progress exists, and refuses to silently restart if that required checkpoint is
missing. Do not add `--samples-per-file`; the Step 8 CLI rejects that cap. The preprocessing fit uses a bounded whole-group reservoir selected
